 ...
 * @author USER
 */
public class AddControllerD extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            AccountBean beansA = new AccountBean();
            Vector<AccountDTO> aList = beansA.getList();
            SceneBean beanS = new SceneBean();
            Vector<SceneDTO> sList = beanS.getListScene();
            request.setAttribute("ADTO", aList);
            request.setAttribute("SDTO", sList);
            String name = request.getParameter("txtFullname");
            request.setAttribute("NAME", name);
        } catch (Exception e) {
            log("ERROR at AddControllerD: " + e.getMessage());
        }
        finally
        {
            request.getRequestDispatcher("addsceneactor.jsp").forward(request, response);
        }
    }
